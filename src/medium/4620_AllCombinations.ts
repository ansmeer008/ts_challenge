...
//풀이 1 - ToUnion 타입과, Exclude 사용함

//문자열을 유니온으로 만들어주는 type
type ToUnion<S> = S extends `${infer F}${infer R}` ? F | ToUnion<R> : never;

type AllCombinations<STR extends string, S extends string = ToUnion<STR>> = [
  S
] extends [never]
  ? ""
